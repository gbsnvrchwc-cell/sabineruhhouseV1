(function () {
  ...
  // ── API Key Management ──────────────────────────────────────────────────────
  var apiKey = sessionStorage.getItem('srh_api_key') || '';
  ...
  function initApiNotice() {
    ...
    saveBtn.addEventListener('click', function () {
      ...
      apiKey = val;
      sessionStorage.setItem('srh_api_key', apiKey);
  ...
  async function callAPI(messages) {
    ...
    if (!res.ok) {
      ...
      var msg = (errData.error && errData.error.message) || ('API error ' + res.status);
      throw new Error(msg);
    }
    ...
    ['arc', 'amazon', 'launch', 'guide'].forEach(function (t) {
      ...
      btn.addEventListener('click', async function () {
        if (!apiKey) {
          ...
          showErr(t, 'Please enter your Anthropic API key above to use the tools.');
          return;
        }
